...
#   curl -fsSL https://raw.githubusercontent.com/vortex3964/CodeDoc/main/install/install.sh | bash -s -- --no-modify-path
#   bash install.sh --local /path/to/CodeDoc
...
write_launcher() {
    ...
    cat > "$BIN_DIR/$APP" <<EOF
#!/usr/bin/env sh
# launcher generated by the $APP installer, rerun the installer to update
...
echo ""
echo "usage:"
echo "  codedoc . -o out           # scan the current directory, write out.md"
echo "  codedoc src/ -o docs/api   # scan src/, write docs/api.md"
echo "  codedoc . -exd test build  # exclude directories"
echo ""
echo -e "${MUTED}open a new terminal (or source your shell config) to use the $APP command${NC}"
echo ""